...
const isAuthenticated = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      decoded = jwt.verify(token, env.ACCESS_TOKEN_SECRET) as Record<
        string,
        any
      >;
    } catch (jwtError: any) {
    ...
    if (!user) {
      throw new ApiError(
        StatusCodes.UNAUTHORIZED,
        errorMessages.TOKEN.TOKEN_USER_NOT_FOUND,
      );
    }
...
const refreshAccessToken = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      decoded = jwt.verify(token, env.REFRESH_TOKEN_SECRET) as Record<
        string,
        any
      >;
    } catch (jwtError: any) {
